...
ADR 001 carries a `tags` field: free-text keywords such as `testing`, `pytest`,
`migration`. `tags` helps a reader who already suspects a decision exists search for
...
`scope` for ADR 001 was authored by reading what it actually binds: `test/`, where the
flat-pytest-function, one-module-per-filter pattern is followed.
